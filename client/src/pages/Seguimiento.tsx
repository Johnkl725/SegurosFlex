...
const SeguimientoSiniestros = () => {
  ...
  useEffect(() => {
    ...
    const fetchSiniestros = async () => {
      try {
        const siniestrosData = await obtenerSiniestrosBeneficiario(usuarioID.toString());
  ...
  const handleSiniestroSeleccionado = (e: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  const handleVerSeguimiento = () => {
    if (!siniestroSeleccionado) {
      setAlerta({ message: "Por favor, selecciona un siniestro", type: "error" });
      return;
    }
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-red-100 via-red-200 to-red-300 p-10">
        <div className="w-full max-w-4xl bg-white p-8 rounded-lg shadow-xl border border-gray-200">
          ...
          <p className="text-gray-600 text-center mb-4">
            Por favor, seleccione un siniestro de la lista para ver el seguimiento detallado.
          </p>
          ...
          <div className="mb-4">
            <label className="block font-medium text-gray-800">Seleccionar Siniestro</label>
            <select
            ...
            >
              ...
              {siniestros.map((siniestro) => (
                <option key={siniestro.siniestroid} value={siniestro.siniestroid}>
                  Número {siniestro.siniestroid} - {siniestro.descripcion} (Fecha: {siniestro.fecha_siniestro})
                </option>
              ))}
            </select>
          </div>
